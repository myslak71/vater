...
def test_search_nip(client):
    ...
    subject, request_id = client.search_nip(nip="3245174504")
...
def test_search_nips(client):
    ...
    subjects, request_id = client.search_nips(
        nips=["3245174504", "1854510877", "7250018312"]
    )
...
def test_search_regon(client):
    ...
    subject, request_id = client.search_regon(regon="887012068")
...
def test_search_regons(client):
    """Test that subject and request identifier are returned for valid test regon."""
    subjects, request_id = client.search_regons(
        regons=["755016841", "216973362", "862391869"]
    )
...
def test_search_account(client):
    ...
    subjects, request_id = client.search_bank_account(
        bank_account="70506405335016096312945164"
    )
...
def test_check_nip(client):
    ...
    is_assigned, request_id = client.check_nip(
        nip="8655104670", bank_account="41146786026458860703735932"
    )
...
def test_check_regon(client):
    ...
    is_assigned, request_id = client.check_regon(
        regon="730371613", bank_account="41146786026458860703735932"
    )
