...
def handle_response2(response, handler_class):
    """TODO"""
    if response.status_code == 400:
        raise InvalidRequestData(ERROR_CODE_MAPPING[response.json()["code"]])
    elif response.status_code != 200:
...
def api_request2(
    ...
    def outer_wrapper(func: Callable) -> Callable:
        ...
        handler = handler_class(url=url_pattern)
        ...
        def inner_wrapper(
            ...
            url = handler.prepare_url(account, date, parameter)
            ...
            return handle_response2(response, handler_class)
...
class CheckRequest:
    ...
    def prepare_url(self, account, date, parameter):
        """TODO"""
...
class Client:
    ...
    @api_request("/api/search/nip/{nip}?date={date}", CheckRequest)
    def search_nip2(self, nip: str, date: datetime.date) -> Tuple[Subject, str]:
